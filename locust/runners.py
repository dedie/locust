    ...
    def spawn_locusts(self, spawn_count=None, wait=False):
        ...
        def hatch():
            ...
            while True:
                if not bucket:
                    logger.info(
                        "All locusts hatched: %s" % ", ".join(
                            ["%s: %d" % (name, count) for name, count
                             in six.iteritems(occurrence_count)]))
...
class LocalLocustRunner(LocustRunner):
    ...
    def start_hatching(self, locust_count=None, hatch_rate=None, wait=False):
        self.hatching_greenlet = gevent.spawn(
            lambda:
            super(LocalLocustRunner, self).start_hatching(locust_count, hatch_rate, wait=wait))
...
class MasterLocustRunner(DistributedLocustRunner):
    ...
    def start_hatching(self, locust_count, hatch_rate):
        num_slaves = (len(self.clients.ready)
        ...
        while True:
            ...
            for client in self.clients.all:
                if client.heartbeat < 0 and client.state != STATE_MISSING:
                    logger.info(
                        'Slave %s failed to send heartbeat, setting state to missing.'
                        % str(client.id))
    ...
    def client_listener(self):
        while True:
            ...
            if msg.type == "client_ready":
                ...
                # warnings.warn("The slave node's clock seem to be out of sync.
                # For the statistics to be correct the different locust servers need to
            ...
            if not self.state == STATE_INIT and all(map(lambda x: x.state != STATE_RUNNING
                                                    and x.state != STATE_HATCHING,
                                                    self.clients.all)):
                self.state = STATE_STOPPED
...
class SlaveLocustRunner(DistributedLocustRunner):
    def __init__(self, *args, **kwargs):
        ...
        # register listener for when all locust users have hatched,
        # and report it to the master node
        def on_hatch_complete(user_count):
        ...
        # spawned locusts to the report that is sent to the master node
        ...
        def on_locust_error(locust_instance, exception, tb):
            formatted_tb = "".join(traceback.format_tb(tb))
            self.client.send(Message("exception", {
                "msg": str(exception),
                "traceback": formatted_tb}, self.client_id))
        events.locust_error += on_locust_error
