...
def diff_response_time_dicts(latest, old):
    ...
    """
    new = {}
    for time_index in latest:
        diff = latest[time_index] - old.get(time_index, 0)
        if diff:
            new[time_index] = diff
    return new
...
class RequestStats(object):
    ...
    def log_request(self, method, name, src_ip, response_time, content_length):
        self.total.log(src_ip, response_time, content_length)
        self.get(name, method).log(src_ip, response_time, content_length)
...
class StatsEntry(object):
    ...
    src_ip = None
    """ Source IP if client IP's are being faked """
    ...
    """
    If set to True, the copy of the response_time dict will be stored in
    response_times_cache every second, and kept for 20 seconds
    (by default, will be CURRENT_RESPONSE_TIME_PERCENTILE_WINDOW + 10).
    We can use this dict to calculate the *current*  median response time,
    as well as other response time percentiles.
    """
    ...
    def log(self, src_ip, response_time, content_length):
        # get the time
        t = int(time.time())

        if (self.use_response_times_cache
                and self.last_request_timestamp and t > self.last_request_timestamp):
    ...
    def _log_response_time(self, response_time):
        ...
        # to avoid too much data that has to be transfered to the master node when
    ...
    def current_rps(self):
        ...
        slice_start_time = max(
            self.stats.last_request_timestamp - 12,
            int(self.stats.start_time or 0))

        reqs = [self.num_reqs_per_sec.get(t, 0)
    ...
    def extend(self, other):
        ...
        self.min_response_time = min(self.min_response_time
                                     or 0, other.min_response_time
                                     or 0) or other.min_response_time
        ...
        for key in other.num_reqs_per_sec:
            self.num_reqs_per_sec[key] = (self.num_reqs_per_sec.get(key, 0)
                                          + other.num_reqs_per_sec[key])
    ...
    def get_current_response_time_percentile(self, percent):
        """
        Calculate the *current* response time for a certain percentile.
        We use a sliding window of (approximately) the last 10 seconds
        (specified by CURRENT_RESPONSE_TIME_PERCENTILE_WINDOW) when calculating this.
        """
        if not self.use_response_times_cache:
            raise ValueError("StatsEntry.use_response_times_cache must be set to True \
                if we should be able to calculate the _current_ response time percentile")
    ...
    def _cache_response_times(self, t):
        ...
        # We'll use a cache size of CURRENT_RESPONSE_TIME_PERCENTILE_WINDOW + 10 since
        # - in the extreme case - we might still use response times (from the cache) for
        # t-CURRENT_RESPONSE_TIME_PERCENTILE_WINDOW-10 to calculate the current response
        # time percentile, if we're missing cached values for the subsequent 20 seconds
...
def on_request_success(request_type, name, src_ip, response_time, response_length, **kwargs):
    global_stats.log_request(request_type, name, src_ip, response_time, response_length)


def on_request_failure(request_type, name, src_ip, response_time, exception, **kwargs):
    global_stats.log_request(request_type, name, src_ip, response_time, 0)
...
def on_slave_report(client_id, data):
    for stats_data in data["stats"]:
        ...
        if request_key not in global_stats.entries:
...
def print_stats(stats):
    console_logger.info((" %-" + str(STATS_NAME_WIDTH)
                         + "s %7s %12s %7s %15s %7s %7s %7s | %7s %7s") % ('Name',
                                                                           '# reqs',
                                                                           '# fails',
                                                                           'T_Stamp',
                                                                           'src_ip',
    ...
    console_logger.info((" %-" + str(STATS_NAME_WIDTH) + "s %7d %12s %42.2f")
...
def print_percentile_stats(stats):
    ...
    console_logger.info(
        (
            ...
            + "s %8s %6s %6s %6s %6s %6s %6s %6s %6s %6s"
            )
        % (
            'Name',
            '# reqs',
...
def requests_csv():
    ...
    for s in chain(sort_stats(runners.locust_runner.request_stats), [runners.locust_runner.stats.total]):
        rows.append('"%s","%s",%i,%i,%i,"%s",%i,%i,%i,%i,%i,%.2f' % (
            s.method,
            s.name,
            s.num_requests,
            s.num_failures,
