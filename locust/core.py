...
class Locust(object):
    ...
    """Function used to calculate waiting time between the
    execution of locust tasks in milliseconds"""
    ...
    weight = 10
    """Probability of locust being chosen. The higher the weight,
    the greater is the chance of it being chosen."""
...
class HttpLocust(Locust):
    """
    Represents an HTTP "user" which is to be hatched and attack the
    system that is to be load tested.
...
class TaskSetMeta(type):
    """
    Meta class for the main Locust class. It's used to allow Locust classes to
    specify task execution
    ...
    *max_wait* milliseconds. It will then schedule another task for execution and so on.
    ...
    def __init__(self, parent):
        ...
        if isinstance(parent, TaskSet):
            ...
            raise LocustError(
                "TaskSet should be called with Locust instance or TaskSet \
                    instance as first argument")

        self.parent = parent

        # if this class doesn't have a min_wait, max_wait or wait_function defined,
        # copy it from Locust
    ...
    def execute_task(self, task, *args, **kwargs):
        # check if the function is a method bound to the current locust,
        # and if so, don't pass self as first argument
...
class TaskSequence(TaskSet):
    ...
    When a TaskSequence starts running, it will pick the task in `index`
    from the *tasks* attribute, execute it, and call its *wait_function*
    which will define a time to sleep for.
    This defaults to a uniformly distributed random number between *min_wait* and
    *max_wait* milliseconds. It will then schedule the `index + 1 % len(tasks)`
    task for execution and so on.

    TaskSequence can be nested with TaskSet, which means that a TaskSequence's *tasks*
    attribute can contain TaskSet instances as well as other TaskSequence instances.
    If the nested TaskSet is scheduled to be executed, it will be
    instantiated and called from the current executing TaskSet. Execution in the
    currently running TaskSet will then be handed over to the nested TaskSet which will
    continue to run until it throws an InterruptTaskSet exception, which is done when
    :py:meth:`TaskSet.interrupt() <locust.core.TaskSet.interrupt>` is called. (execution
    ...
    In this class, tasks should be defined as a list, or simply define the tasks with the
    @seq_task decorator
    """
