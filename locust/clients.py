...
from requests_toolbelt.adapters.source import \
    SourceAddressAdapter  # For source IP
...
class HttpSession(requests.Session):
    ...
    def request(self, method, url, src_ip=None, name=None, catch_response=False, **kwargs):
        """
    ...
    def _send_request_safe_mode(self, method, url, src_ip, **kwargs):
        """
        ...
        if src_ip is not None:
            session.mount(self, 'http://', SourceAddressAdapter(src_ip))
            session.mount(self, 'https://', SourceAddressAdapter((src_ip, 8999)))
        try:
